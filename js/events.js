...
function _newsSetup(begin, end, pagination){
	...
	$.getJSON('content/events/' + params.lang + '.json', function(data){
      $("#posts").html('');
      ...
      $.getJSON('content/events/' + defaultLang + '.json', function(data){
        $("#posts").html('');
...
function eventsSetup(begin, end, pagination){
...
function createNews(data, begin, end, pagination) {
	...
	if(pagination) {
		...
		if(totalPages > 1) {
			...
			else html = "<ul class='pagination' role='menubar' aria-label='Pagination'><li class='arrow'><a onclick='newsSetup(" + (current-2)*step + ", " + (current-1)*step + ", true)'>&laquo; Previous</a></li>";

	  		if(current == 1) html += "<li class='current'><a>1</a></li>";
	  		else html += "<li class=''><a onclick='newsSetup(0," + step + ", true);'>1</a></li>";
	  					
	  		if(totalPages > 1) {
				if(current == 2) html += "<li class='current'><a>2</a></li>";
		  		else html += "<li class=''><a onclick='newsSetup(" + step + "," + step*2 + ", true);''>2</a></li>";
	  		}		

	  		if(totalPages > 2) {
	  			for(var i = 3; i <= totalPages; i++) {
	  				if(current == i) html += "<li class='current'><a>" + i + "</a></li>";
		  			else html += "<li class=''><a onclick='newsSetup(" + step*(i-1) + "," + step*i + ", true);''>" + i + "</a></li>";
	  			}
	  		}

	  		if(totalPages == current) html += "<li class='arrow unavailable' aria-disabled='true'><a>Next &raquo;</a></li></ul>";
	  		else html += "<li class='arrow'><a onclick='newsSetup(" + current*step + ", " + (current+1)*step + ", true)'>Next &raquo;</a></li></ul>";
	  	}
